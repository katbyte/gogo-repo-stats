...
func CmdFetch(cmd *cobra.Command, args []string) error {
	...
	// TODO for quick mode
	// get each PR in the database that isn't closed and update its cache
	// this way when we list them below we can stop after the first cached PR
	...
	err = r.ListAllPullRequests("all", func(prs []*github.PullRequest, resp *github.Response) error {
		for i, p := range prs {
			...
			daysOpen, daysWaiting, daysToFirst, err := cache.ComputeAndUpdatePRStats(pr.GetNumber())
			if err != nil {
